    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2 shrink-0 font-bold text-xl text-[#1a1a1a]">
            <Sun className="w-6 h-6 text-educture-orange" strokeWidth={2.5} />
            Educture
          </Link>
            ...
            {links.map((link) => (
              <Link
                ...
                className={`hover:text-educture-orange transition-colors ${
                  location.pathname === link.to ? 'text-educture-orange font-semibold' : ''
                }`}
              >
                {link.label}
              </Link>
